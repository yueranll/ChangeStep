...
class MiMotion():
    ...
    def login(self, phone, password):
        ...
        try:
            r1 = requests.post(url1, data=data1, headers=self.headers, timeout=10).json()
            if "access" in r1:
                code = r1["access"]
            else:
                # 改进：区分密码错误和账号不存在
                err_msg = r1.get('error_description', '未知错误')
                return 0, 0, f"【登录失败】接口返回: {err_msg}"
        except Exception as e:
        ...
        try:
            ...
            return 0, 0, "【Token获取失败】服务器未返回有效的登录令牌"
        except Exception as e:
            return 0, 0, f"【请求失败】二级登录接口异常: {e}"
    ...
    def main(self):
        ...
        try:
            ...
            max_step = int(self.check_item.get("max_step", 29999))
        except (ValueError, TypeError):
            print(f"[*] 账号 {phone} 步数参数格式错误，使用默认范围 20000-29999")
        ...
        try:
            ...
            response = requests.post(url=url, data=data, headers=headers, timeout=15).json()

            # 改进：根据响应结果判定成功/失败
            if response.get("message") == "success":
                return f"✅ 账号: {phone} | 步数: {step} | 结果: 成功"
            else:
                return f"⚠️ 账号: {phone} | 步数: {step} | 结果: {response.get('message', '未知返回')}"

        except IndexError:
            return f"❌ 账号: {phone} | 错误: 数据匹配失败（正则未找到指定位置）"
        except Exception as e:
            return f"❌ 账号: {phone} | 系统异常: {e}"


if __name__ == "__main__":
    # 适配 GitHub Actions 环境变量读取
    accounts_env = os.getenv("MI_USER")
    if not accounts_env:
        print("!! 错误: 未检测到 MI_USER 环境变量，请在 GitHub Secrets 中配置。")
        exit(1)
